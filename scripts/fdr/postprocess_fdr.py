...
def aggregate_groundtruth_results(root_dir : str,
    ...
    for subdir in os.listdir(root_dir):
        ...
        if os.path.isdir(subdir_path) and subdir in tissue_list:
            ...
            batch_dir = os.path.join(subdir_path, groundtruth_name)
            ...
            if os.path.isdir(approx_dir):
                ...
                for num_tfs in tf_list:
                    for num_non_tfs in non_tf_list:
                        ...
                        gt_path = os.path.join(subdir_path, groundtruth_dir, "groundtruth_grn.csv")
                        ...
                        if apply_benjamini_hb:
                            _, pvals_corrected_approx, _, _ = multipletests(approx_grn['pvalue'], method='fdr_bh')
                            _, pvals_corrected_gt, _, _ = multipletests(gt_grn['pvalue'], method='fdr_bh')
                            approx_grn.drop(columns=['pvalue'], inplace=True)
                            gt_grn.drop(columns=['pvalue'], inplace=True)
                            approx_grn['pvalue'] = pvals_corrected_approx
                            gt_grn['pvalue'] = pvals_corrected_gt
                        
                        if adjust_pvalues:
                            gt_max_occurence = max(gt_grn['shuffled_occurences'])
                            approx_max_occurence = max(approx_grn['shuffled_occurences'])
                            gt_grn['pvalue_adj'] = gt_grn['pvalue'] * (gt_max_occurence+1.0) / (gt_grn['shuffled_occurences']+1.0)
                            approx_grn['pvalue_adj'] = approx_grn['pvalue'] * (approx_max_occurence+1.0) / (approx_grn['shuffled_occurences']+1.0)
                            gt_grn.drop(columns=['pvalue'], inplace=True)
                            approx_grn.drop(columns=['pvalue'], inplace=True)
                            gt_grn.rename(columns={"pvalue_adj" : 'pvalue'}, inplace=True)
                            approx_grn.rename(columns={"pvalue_adj" : 'pvalue'}, inplace=True)
                        ...
                        corr = pearsonr(gt_pvals, approx_pvals)[0]
                        ...
                        tn, fp, fn, tp = confusion_matrix(gt_001, approx_001, labels=[0,1]).ravel()
                        
                        # Compute runtime savings.
                        with open(time_path, "r") as f:
                            approx_runtime = float(f.read().strip())
                        gt_time_path = os.path.join(subdir_path, groundtruth_dir, "groundtruth_runtime.txt")
                        ...
                        gt_em_path = os.path.join(subdir_path, groundtruth_dir, "groundtruth_emissions.txt")
                        ...
                        # Save results.
                        tissue_dict[(num_non_tfs, num_tfs)] = {'mae': mae, 
                                                               'f1_005': f1_005, 
                                                               'f1_001': f1_001,
    ...
    for subdir in os.listdir(root_dir):
        ...
        if os.path.isdir(subdir_path) and subdir in tissue_list:
            ...
            for key, val in res_dict.items():
                ...
                all_results_dict['correlation'].append(correlation)
                all_results_dict['tp_001'].append(tp_001)
    
    all_res_df = pd.DataFrame(all_results_dict)
    all_res_df.to_csv(f'./{out_file_name}', index=False)
...
if __name__ == "__main__":
    ...
    ### Version 2 ###
    # Approx. FDR results computation.
    non_tf_list = list(range(1, 10, 1)) + list(range(10, 101, 10))
    #non_tf_list = [3,4,5,6,7,8,9,10,20,30,40,50,70,100]
    tf_list = [-1]
